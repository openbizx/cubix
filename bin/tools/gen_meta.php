...
$moduleDir = Openbizx::$app->getModulePath().DIRECTORY_SEPARATOR.str_replace(".",DIRECTORY_SEPARATOR,$module);
...
for ($i=0; $i<count($namings); $i++) {
	echo ($i+1).". module path: ".str_replace(Openbizx::$app->getModulePath(),"",$namings[$i][0]).
...
while(1) {
	...
	if(strtolower($selection)=='s'){
		...
		$custom_opts[0] = Openbizx::$app->getModulePath().DIRECTORY_SEPARATOR.trim($module);
...
$modFile = $moduleDir = Openbizx::$app->getModulePath() . "/" . $modFolder."/mod.xml";
