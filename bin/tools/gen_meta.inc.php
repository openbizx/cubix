...
class MetaGenerator
{
    ...
    function __construct($module, $dbname, $table, $opts)
    {
        ...
        $this->table = $table;
        $this->opts = $opts;
        $this->dbConfig = Openbizx::$app->getConfiguration()->getDatabaseInfo($dbname);
    }
...
class DashboardGenerator
{
	...
	public function generateDashboardWidget($table)
	{
        ...
        $targetPath = $moduleDir = Openbizx::$app->getModulePath() . "/" . getModuleName($this->module)."/widget";
        ...
        if(file_exists($targetFile)){
			if(CLI){echo "\t".str_replace(Openbizx::$app->getModulePath(),"",$targetFile)." exists skipped." . PHP_EOL. PHP_EOL;}
			return ; 
        }            
        ...
        if(CLI){echo "\t".str_replace(Openbizx::$app->getModulePath(),"",$targetFile)." is generated." . PHP_EOL. PHP_EOL;}
        return $targetFile;		
	}
	...
	public function generateDashboardView($table)
	{
        ...
        $targetPath = $moduleDir = Openbizx::$app->getModulePath() . "/" . getModuleName($this->module)."/view";
        ...
        if(file_exists($targetFile)){
			if(CLI){echo "\t".str_replace(Openbizx::$app->getModulePath(),"",$targetFile)." exists and skipped." . PHP_EOL. PHP_EOL;}
			return ; 
        }    
	...
	public function modifyViewTpl()
	{
        ...
        $targetPath = $moduleDir = Openbizx::$app->getModulePath() . "/" . getModuleName($this->module)."/template";
...
';        
        ...
        if(!preg_match("/widget\.LeftMenu/si",$content)){
			...
			if(CLI){echo "\t".str_replace(Openbizx::$app->getModulePath(),"",$targetFile)." was modified and skipped." . PHP_EOL. PHP_EOL;}
			return ;
        }
        

        file_put_contents($targetFile, $content);
        if(CLI){echo "\t".str_replace(Openbizx::$app->getModulePath(),"",$targetFile)." is modified." . PHP_EOL. PHP_EOL;}
        return $targetFile;				
	}
...
class DOGenerator
{
    ...
    {
        $db = Openbizx::$app->getDbConnection($this->db_name);
        if (!$db)
        {
    ...
    public function generateDO()
    {
        ...
        $targetPath = $moduleDir = Openbizx::$app->getModulePath() . "/" . str_replace(".", "/", $this->module) . "/do";
        if (!file_exists($targetPath))
        {
            if(CLI){echo "Create directory $targetPath" . PHP_EOL;}
            mkdir($targetPath, 0777, true);
        ...
        file_put_contents($targetFile, $content);
        if(CLI){echo "\t".str_replace(Openbizx::$app->getModulePath(),"",$targetFile)." is generated." . PHP_EOL;}
        return $targetFile;
    }
...
class FormGenerator
{
     ...
     * @var string
     */
    public $form_obj_class = "Openbizx\Easy\EasyForm";
    /**
     * ACL name
     * @var string
     */
    ...
    public function generateForm($form_name, $tpl_file)
    {
        ...
        $targetPath = $moduleDir = Openbizx::$app->getModulePath() . "/" . str_replace(".", "/", $this->module) . "/form";
        if (!file_exists($targetPath))
        {
            if(CLI){echo "Create directory $targetPath" . PHP_EOL;}
            mkdir($targetPath, 0777, true);
...
class ModGenerator
{
    ...
    function generateMod($table_name)
    {
        ...
        $targetPath = $moduleDir = Openbizx::$app->getModulePath() . "/" . $module_name;
        if (!file_exists($targetPath))
        {
            if(CLI){echo "Create directory $targetPath" . PHP_EOL;}
            mkdir($targetPath, 0777, true);
	...
	function modifyMod($table_name)
    {
        ...
        $module_name = getModuleName($this->module);
        $targetFile = $moduleDir = Openbizx::$app->getModulePath() . "/" . getModuleName($this->module)."/mod.xml";
        ...
        //save files
        file_put_contents($targetFile, $content);
        if(CLI){echo "\t".str_replace(Openbizx::$app->getModulePath(),"",$targetFile)." is modified." . PHP_EOL;}
        return $targetFile;
    }    
...
{
    $targetPath = $moduleDir = Openbizx::$app->getModulePath() . "/" . getModuleName($module) . "/template";
    if (!file_exists($targetPath))
        mkdir($targetPath, 0777, true);
