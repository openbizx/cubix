...
$db = Openbizx::$app->getDbConnection($cubiInstallDb);
if (!$db) {
	echo "Please create a CubiInstall database in cubi/Config.xml.".PHP_EOL;
}

$modules = array ('system','menu');
foreach (glob(Openbizx::$app->getModulePath().DIRECTORY_SEPARATOR."*") as $dir){
	$modName = str_replace(Openbizx::$app->getModulePath().DIRECTORY_SEPARATOR,"",$dir);
