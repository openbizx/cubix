...
if($argv[1]){
	$modulename= $argv[1];
	$dir_iterator = new RecursiveDirectoryIterator(Openbizx::$app->getModulePath().'/'.$modulename);
}else{
	$dir_iterator = new RecursiveDirectoryIterator(Openbizx::$app->getModulePath());
}
...
foreach ($iterator as $file) {
    if(preg_match("/.*?List.*?\.xml$/si",$file)){		    	
    	echo "Testing file: " .str_replace(Openbizx::$app->getModulePath()."/","",$file)." ";
