...
class InstallerService extends PackageService
{
	...
	{
		$uri = addslashes($uri);
		$repoRec = Openbizx::getObject("market.repository.do.RepositoryDO")->fetchOne("[repository_uri]='$uri'");
		if($repoRec)
		{
	...
	protected function downloadUrl($url, $filename)
    {
        ...
        $total = 0;
        $contents = '';
        $utilService = Openbizx::getService("service.utilService");
        while (!feof($handle)) {
    ...
    {   
        ...
        $operator = Openbizx::$app->getProfile()->getProfileName(Openbizx::$app->getUserProfile("Id"),'short');  
    ...
    {	                                	
        $pkgDo = Openbizx::getObject(self::INSTALLED_DO);        
	...
	{
	    if (CLI) echo $text.nl;
	    //Openbizx::$app->getLog()->log(LO_ERR, "ECHO", $text);
