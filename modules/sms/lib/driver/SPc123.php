 ...
 */
	public function send($mobile,$content,$schedule=null)
	{
		...
		if($recinfo!=100)
		{				
			Openbizx::getService(LOG_SERVICE)->log(LOG_ERR,"SMS","sendMessage: ". $content." c123：".$mobile.':'.$recinfo);
			return false;
		}
		else
		{
    ...
    public function getMsgBalance()
    {
		...
		$recinfo= Openbizx::getService("sms.lib.SmsUtilService")->curl($this->url_mm,$Param);
		$recArr=explode('||',$recinfo);
		if($recArr[0]!=100)
		{
			Openbizx::getService(LOG_SERVICE)->log(LOG_ERR,"SMS","getSentMessageCount c123: ".$recinfo);
			return false;
		}
		else
		{
