...
*/
	public function send($mobile,$content,$schedule=null)
	{
		...
		$recinfo= Openbizx::getService("sms.lib.SmsUtilService")->getHttpResponse($url);
		parse_str($recinfo,$recArr);
		if($recArr['errid']!=1)
		{	
			Openbizx::getService(LOG_SERVICE)->log(LOG_ERR,"SMS","sendMessage: ". $recArr['msg'].' 18dx:'.$mobile.':'.$recinfo);
			return false;
		}
		else
		{
    ...
    {	
		...
		$recinfo= Openbizx::getService("sms.lib.SmsUtilService")->curl($this->url,$Param);
		$errorInfo=$this->getMsg($recinfo);
		if($errorInfo)
		{			
			Openbizx::getService(LOG_SERVICE)->log(LOG_ERR,"SMS","getSentMessageCount 18dx:".$errorInfo);
			return false;
		}
		else
		{
