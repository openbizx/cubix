...
class SmsService extends MetaObject
{
    ...
    {
		 foreach(Openbizx::getObject($this->smsProviderDO)->directFetch("[status]=1") as $providerRec)
		 {	
	...
	public function SendSmsFromQueue($SmsQueue=null,$limit=10){
		$SmsProviderDO = Openbizx::getObject($this->smsProviderDO);
	...
	protected function _updateSmsQueueStatus($action,$id)
	{
		...
		{
			case 'pending':
				$return= Openbizx::getObject($this->smsQueueDO)->updateRecords("[status]='pending'","[Id]={$id}");
				 break;
			case 'sending':
				$return= Openbizx::getObject($this->smsQueueDO)->updateRecords("[status]='sending'","[Id]={$id}");
				 break;
			case 'sent':
				$return= Openbizx::getObject($this->smsQueueDO)->updateRecords("[status]='sent'","[Id]={$id}");
				 break;
			case 'batch_sending':	 			    
				$return= Openbizx::getObject($this->smsQueueDO)->updateRecords("[status]='sending'","[Id] ".Openbizx::getService("sms.lib.SmsUtilService")->db_create_in($sms_ids));
				 break;
		}
		return $return;
	}
	...
	{
		$SmsQueueDO = Openbizx::getObject($this->smsQueueDO);
		if(!$this->validateMobile($mobile))
		{
			return false;
		}
 ...
 */
	protected  function _getSmsPreference(){
		...
		$PreferenceDO = Openbizx::getObject($this->preferenceDO);
 ...
 */
	protected function _getProvider(){
		...
		if(!$SmsProviderInfo)
		{
			Openbizx::getService(LOG_SERVICE)->log(LOG_ERR,"SMS","No available provider found");
			return false;
		}
		...
		if(!is_object($obj))
		{
			Openbizx::getService(LOG_SERVICE)->log(LOG_ERR,"SMS","Cannot load provider driver :".$SmsProviderArr['driver']);			
			return false;
		}		
		return $obj;
	}
 ...
 */
	...
	{		
		...
		$driverrFile=Openbizx::$app->getModulePath().'/'.$FileName.'.php';
		if(!file_exists($driverrFile))
		{
			Openbizx::getService(LOG_SERVICE)->log(LOG_ERR,"SMS","Cannot load provider driver :".$ProvidersInfo['driver']);			
			return false;
		}
		else
		{
