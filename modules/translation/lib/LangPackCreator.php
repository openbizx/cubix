...
class LangPackCreator
{
    ...
    public function createNew($translate = false)
    {
		...
		if ($this->systemOnly == false && strpos($this->module, 'themes/') === false) {
	    	...
	    	foreach (glob(Openbizx::$app->getModulePath().DIRECTORY_SEPARATOR.$this->module,GLOB_ONLYDIR) as $dir)
	    	{
	    		$module_name = str_replace(Openbizx::$app->getModulePath().DIRECTORY_SEPARATOR,"",$dir);
    ...
    {
    	$strings = array();
    	$dir = Openbizx::$app->getModulePath().DIRECTORY_SEPARATOR.$module;
    	$filelist = $this->getFileList($dir,".xml");
    	libxml_use_internal_errors(true);
    	foreach($filelist as $file){
    		$fileArr = explode(DIRECTORY_SEPARATOR,str_replace(Openbizx::$app->getModulePath().DIRECTORY_SEPARATOR,"",$file));
    		$prefix = "";
    		...
    		if(CLI){
				echo "   Analyst XML File : ".str_replace(Openbizx::$app->getModulePath().DIRECTORY_SEPARATOR,"",$file)." ".PHP_EOL;
			}
    ...
    private function getStringsFromMsg($module,$dir=null)
    {
    	...
    	foreach($filelist as $file){
    		if(CLI){
				echo "   Analyst Message File: ".str_replace(OPENBIZ_APP_PATH.DIRECTORY_SEPARATOR,"",str_replace(Openbizx::$app->getModulePath().DIRECTORY_SEPARATOR,"",$file))." ".PHP_EOL;
			}			
    ...
    private function getStringsFromTemplate($module,$dir=null)
    {
    	...
    	foreach($filelist as $file){
    		if(CLI){
				echo "   Analyst Template File: ".str_replace(OPENBIZ_APP_PATH.DIRECTORY_SEPARATOR,"",str_replace(Openbizx::$app->getModulePath().DIRECTORY_SEPARATOR,"",$file))." ".PHP_EOL;
			}			
    ...
    private function getStringsFromMenu()
    {
    	...
    	$menuDO = Openbizx::getObject($menuDoName,1);
    ...
    private function getStringsFromACL()
    {
    	...
    	$ACLDO = Openbizx::getObject($ACLDoName,1);
