...
class MetaObjExport
{
	...
	public function GetDocDocument()
    {
        if ($this->doc) 
            return $this->doc;
        $this->xmlFile = Openbizx::$app->getModulePath()."/".str_replace(".","/",$this->object->objectName).".xml";
	...
	{
		if (is_a($this->object, "Openbizx\Easy\EasyForm")) {
			$this->objType = "Form";
			return $this->Form2XML();
		}
		else if (is_a($this->object, "Openbizx\Easy\WebPage")) {
			$this->objType = "View";
			return $this->View2XML();
		}
		else if (is_a($this->object, "Openbizx\Data\BizDataObj")) {
	...
	protected function DataObj2XMLElement($obj, $clz='')
	{
		...
		if ($className == "Openbizx\Data\Tools\TableJoin") {
			$elemName = "Join";
		}
		if ($className == "Openbizx\Data\Tools\BizRecord") {
			$vars = $obj;
			$elemName = "BizFieldList";
		}
		if ($className == "Openbizx\Object\MetaIterator") {
			$vars = $obj;
		}
		if (is_subclass_of($obj, "Openbizx\Data\BizDataObj")) {
			$elemName = "BizDataObj";
		}
		if ($clz!='') {
			$elemName = $clz;
	...
	protected function ViewObj2XMLElement($obj)
	{
		...
		if (is_subclass_of($obj, "Openbizx\Easy\WebPage")) {
			$elemName = "WebPage";
		}
