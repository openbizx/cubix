...
class ModuleLoader
{
    ...
    {
        return Openbizx::$app->getDbConnection($this->dbName);
    }
    ...
    public function loadModule($installSql = false)
    {
        ...
        Openbizx::getObject("system.do.ModuleCachedDO")->cleanCache();
        return true;
    }
    ...
    public function upgradeModule($forceUpgrade = false)
    {
        ...
        $modFolder = Openbizx::$app->getModulePath() . "/" . $this->objectName;
    ...
    static public function isModuleOld($module, $dbName = null)
    {
        ...
        $modFolder = Openbizx::$app->getModulePath() . "/" . $module;
        $modfile = $modFolder . "/mod.xml";
        $xml = simplexml_load_file($modfile);
        $mod_ver = $xml['Version'];
    ...
    protected function upgradeSQLs($baseVersion, $targetVersion)
    {
        ...
        $upgradeFile = Openbizx::$app->getModulePath() . "/" . $this->objectName . "/upgrade.xml";
    ...
    {
        $this->log("Install Module Sql.");
        $sqlfile = Openbizx::$app->getModulePath() . "/" . $this->objectName . "/mod.install.sql";
        if (!file_exists($sqlfile)) {
            return true;
        }
    ...
    protected function installResource($xml)
    {
        ...
        if (isset($xml->Files) && isset($xml->Files->Copy)) {
            foreach ($xml->Files->Copy as $copy) {
                // echo "Copy ".Openbizx::$app->getModulePath().'/'.$this->objectName.'/'.$copy['From'].' > '.OPENBIZ_APP_PATH.'/'.$copy['ToDir'].PHP_EOL;
                $toDirs = glob(OPENBIZ_APP_PATH . '/' . $copy['ToDir']);
                //print_r($toDirs);
                $fromFiles = glob(Openbizx::$app->getModulePath() . '/' . $this->objectName . '/' . $copy['From']);
                //print_r($fromFiles);
    ...
    protected function installWidgets($xml)
    {
        ...
        if (isset($xml->Widgets) && isset($xml->Widgets->Widget)) {
            ...
            //clean  obj cache
            $menuObj = Openbizx::getObject("system.do.WidgetDO");
            $menuObj->CleanCache();
    ...
    protected function loadWidget($widget, $moduleName = '')
    {
        ...
        $configable = Openbizx::getObject($name)->configable;
        $configable = (int) $configable;

        $do = Openbizx::getObject("system.do.WidgetDO");
        $recArr = array(
            "name" => $name,
            "module" => $module,
            "title" => $title,
    ...
    protected function installEventObservers($xml)
    {
        ...
        if (isset($xml->EventObservers) && isset($xml->EventObservers->Observer)) {
            ...
            //clean  obj cache
            $obsObj = Openbizx::getObject("eventmgr.do.EventObserverDO");
            $obsObj->CleanCache();
    ...
    protected function loadObserver($observer, $moduleName = '')
    {
        ...
        $do = Openbizx::getObject("eventmgr.do.EventObserverDO");
        $recArr = array(
            "name" => $name,
    ...
    protected function copyResourceFiles()
    {
        ...
        $module = $this->objectName;
        $modulePath = Openbizx::$app->getModulePath() . "/$module";
    ...
    protected function installMetaDo()
    {
        ...
        foreach ($g_MetaFiles as $metaFile) {
            $metaName = str_replace('/', '.', str_replace(array(Openbizx::$app->getModulePath() . '/', '.xml'), '', $metaFile));
            // load do
